...
const createRide = async (decodedToken, payload) => {
    if (!decodedToken) {
...
const setRideStatus = async (rideId, status, decodedToken) => {
    ...
    if (!Object.values(ride_interfaces_1.IRideStatus).includes(status)) {
    ...
    if (!allowedStatuses.includes(status)) {
        throw new appError_1.AppError(http_status_codes_1.default.FORBIDDEN, `You are not allowed to set status to ${status}. Only Driver can do this.`);
    }
    ...
    if (status === ride_interfaces_1.IRideStatus.CANCELED &&
...
const getMyRide = async (decodedToken) => {
    const myRides = await ride_model_1.Ride.find({ rider: decodedToken.userId }).populate("driver", "-_id name phone");
